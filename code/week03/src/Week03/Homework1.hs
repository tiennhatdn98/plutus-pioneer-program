...
-- not use unstable for production
...
mkValidator dat _ ctx
    | checkDeadline == True = traceIfFalse "This isn't beneficary 2" checkSig (beneficiary2 data)
    | otherwise = traceIfFalse "This isn't beneficary 1" checkSig (beneficiary1 data)
    where
        info :: TxInfo
