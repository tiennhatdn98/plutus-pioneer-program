...
mkValidator pkh s () ctx =
    ...
    -- traceIfFalse :: BuiltinString -> Bool -> Bool: Emit the given BuiltinString only if the argument evaluates to False.
  where
    info :: TxInfo
    ...
    -- elem :: (Foldable t, Eq a) => a -> t a -> Bool: Return True if the list contains an item equal to the first argument
    -- txInfoSignatories :: PubKeyHash: Signatures provided with the transaction, attested that they all signed the tx

    checkDeadline :: Bool
    checkDeadline = from s `contains` txInfoValidRange info
    -- from :: a -> Interval a: all values that are greater than or equal to a. [a, +oo)
    -- contains :: Ord a => Interval a -> Interval a -> Bool: a contains b is true if the Interval b is entirely contained in a. That is, a contains b if for every entry s, if member s b then member s a.
...
typedValidator p = Scripts.mkTypedValidator @Vesting
  ...
  -- applyCode: Apply a compiled function to a compiled argument.
  where
